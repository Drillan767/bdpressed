...
describe('PENDING_PAYMENT Order State Transitions', function () {
    ...
    describe('PENDING_PAYMENT → PAID', function () {
        ...
        it('allows single item order to transition from PENDING_PAYMENT to PAID', function () {
            ...
            OrderStateTestHelpers::assertEmailSent(OrderConfirmationNotification::class);
        });
        ...
        it('sends email to both user and admin on payment confirmation', function () {
            $order = OrderStateTestHelpers::createSingleItemOrder(OrderStatus::PENDING_PAYMENT);
            
            $order->transitionTo(OrderStatus::PAID);
            
            // Should send confirmation to both user and admin
            OrderStateTestHelpers::assertEmailSent(OrderConfirmationNotification::class);
        });

        it('updates inventory when transitioning to PAID for item orders', function () {
            $order = OrderStateTestHelpers::createSingleItemOrder(OrderStatus::PENDING_PAYMENT);
